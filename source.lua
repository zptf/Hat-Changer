...
local ThunderMadeLibrary = {
	Flags = {},
	Theme = {
		Default = {
			TextFont = "Default", -- Default will use the various font faces used across ThunderMade
		...
		Light = {
			TextFont = "Gotham", -- Default will use the various font faces used across ThunderMade
...
local ThunderMade = game:GetObjects("rbxassetid://10804731440")[1]



if gethui then
	ThunderMade.Parent = gethui()
elseif syn.protect_gui then 
	syn.protect_gui(ThunderMade)
	ThunderMade.Parent = CoreGui
elseif CoreGui:FindFirstChild("RobloxGui") then
	ThunderMade.Parent = CoreGui:FindFirstChild("RobloxGui")
else
	ThunderMade.Parent = CoreGui
end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == ThunderMade.Name and Interface ~= ThunderMade then
			Interface.Enabled = false
			Interface.Name = "ThunderMade-Old"
		end
	end
else
...
local Main = ThunderMade.Main
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList

ThunderMade.DisplayOrder = 100
...
local Notifications = ThunderMade.Notifications

local SelectedTheme = ThunderMadeLibrary.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = ThunderMadeLibrary.Theme[ThemeName]
	for _, obj in ipairs(ThunderMade:GetDescendants()) do
	...
	ThunderMade.Main.BackgroundColor3 = SelectedTheme.Background
	ThunderMade.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	ThunderMade.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	ThunderMade.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow
	
	ThunderMade.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	ThunderMade.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	ThunderMade.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	table.foreach(Data, function(FlagName, FlagValue)
		if ThunderMadeLibrary.Flags[FlagName] then
			spawn(function() 
				if ThunderMadeLibrary.Flags[FlagName].Type == "ColorPicker" then
					ThunderMadeLibrary.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if ThunderMadeLibrary.Flags[FlagName].CurrentValue or ThunderMadeLibrary.Flags[FlagName].CurrentKeybind or ThunderMadeLibrary.Flags[FlagName].CurrentOption or ThunderMadeLibrary.Flags[FlagName].Color ~= FlagValue then ThunderMadeLibrary.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
			ThunderMadeLibrary:Notify({Title = "Flag Error", Content = "ThunderMade was unable to find '"..FlagName.. "'' in the current script"})
		end
	end)
end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(ThunderMadeLibrary.Flags) do
...
function ThunderMadeLibrary:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= ThunderMadeLibrary.Theme.Default then
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("ThunderMade | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
		...
		if ThunderMade.Name == "ThunderMade" then
...
function Hide()
	Debounce = true
	ThunderMadeLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping on V", Duration = 7})
...
function ThunderMadeLibrary:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "ThunderMade Interface Suite"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by Sirius"
	if Settings.LoadingTitle ~= "ThunderMade Interface Suite" then
		LoadingFrame.Version.Text = "ThunderMade UI"
	end
	...
	pcall(function()
		...
		if not isfolder(ThunderMadeFolder.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(ThunderMadeFolder.."/Discord Invites") then
			makefolder(ThunderMadeFolder.."/Discord Invites")
		end
		if not isfile(ThunderMadeFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(ThunderMadeFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"ThunderMade RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(ThunderMadeFolder.."/Key System") then
			makefolder(ThunderMadeFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			...
			if not Success then
				print("ThunderMade | "..Settings.KeySettings.Key.." Error " ..tostring(Response))
			end
		end
		...
		if isfile(ThunderMadeFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			if readfile(ThunderMadeFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) == Settings.KeySettings.Key then
				Passthrough = true
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				if KeyMain.Input.InputBox.Text == Settings.KeySettings.Key then
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(ThunderMadeFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, Settings.KeySettings.Key)
						end
						ThunderMadeLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
					end
				else
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("ThunderMade | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					ThunderMadeLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("ThunderMade | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					if Dropdown.Selected.Text ~= Option then
						...
						if not Success then
							...
							print("ThunderMade | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					ThunderMadeLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end

			return DropdownSettings
		enda3925009a0562d149e069cbc3c2b2fa322fc56199
		...
		function Tab:CreateKeybind(KeybindSettings)
			...
			UserInputService.InputBegan:Connect(function(input, processed)
				...
				if CheckingForKey then
					if input.KeyCode ~= Enum.KeyCode.Unknown and input.KeyCode ~= Enum.KeyCode.Alt then
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("ThunderMade | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					ThunderMadeLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("ThunderMade | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					ThunderMadeLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if Dragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("ThunderMade | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					ThunderMadeLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
...
function ThunderMadeLibrary:Destroy()
	ThunderMade:Destroy()
end
...
UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode.Alt and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function ThunderMadeLibrary:LoadConfiguration()
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				LoadConfiguration(readfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension))
				ThunderMadeLibrary:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end

return ThunderMadeLibrary
